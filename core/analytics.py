...
from .models import Appointment, ClientVisit, WalkIn
...
def business_summary(start_date, end_date):
    ...
    appointments = Appointment.objects.filter(appointment_date__range=(start_date, end_date)).exclude(status=Appointment.CANCELLED)
    ...
    visits = ClientVisit.objects.filter(visited_at__date__range=(start_date, end_date))
